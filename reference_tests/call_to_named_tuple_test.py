...
class ReferenceTest(reference_test_base.TestCase):
  ...
  def test_inline(self):
    self.assertNativeMatchesCompiled(inline_namedtuple, 1)

  def test_external(self):
    nt = collections.namedtuple('TestNamedTuple', ('a', 'b'))
    self.assertNativeMatchesCompiled(external_namedtuple, 1, nt)

  def test_subclass(self):
    self.assertNativeMatchesCompiled(namedtuple_subclass, 1)
